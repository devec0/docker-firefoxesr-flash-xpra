...
# firefox, alsa, flash
RUN yum -y install flash-plugin nspluginwrapper alsa-plugins-pulseaudio libcurl firefox
...
RUN yum -y install gcc python-devel libXtst-devel libXdamage-devel \
    ...
    redhat-rpm-config
RUN yum -y install xorg-x11-server-Xorg xorg-x11-drv-dummy xorg-x11-xauth \
